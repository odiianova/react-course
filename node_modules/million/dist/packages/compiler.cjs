...
const displayIntro = (options) => {
  ...
  let message = `
  ${colors.bold(
    colors.magenta(`\u26A1 Million.js ${"3.0.3" }`)
  )}
...
function pushExpression(state, expr) {
  ...
  state.exprs.push(t__namespace.jsxAttribute(t__namespace.jsxIdentifier(key), t__namespace.jsxExpressionContainer(t__namespace.cloneNode(expr))));
  return key;
}
...
function transformJSXForElement(ctx, path) {
  if (isJSXForElement(ctx, path)) {
    path.node.openingElement.attributes.push(t__namespace.jsxAttribute(t__namespace.jsxIdentifier("scoped")));
  }
}
...
function extractJSXExpressions(state, path, top) {
  ...
  if (isPathValid(path, t__namespace.isJSXFragment)) {
    path.replaceWith(t__namespace.jsxElement(
...
function transformJSX(ctx, path) {
  ...
  extractJSXExpressions(state, path, !(isPathValid(path.parentPath, t__namespace.isJSXElement) || isPathValid(path.parentPath, t__namespace.isJSXFragment) || isPathValid(path.parentPath, t__namespace.isJSXAttribute)));
  ...
  const newComponent = t__namespace.arrowFunctionExpression(
    args,
    path.node
  );
  const options = [
    ...
    t__namespace.objectProperty(
      ...
      t__namespace.stringLiteral(id.name)
    )
  ];
  if (state.keys.length) {
    options.push(t__namespace.objectProperty(
      t__namespace.identifier("portals"),
      t__namespace.arrayExpression(state.keys)
    ));
  }
  ...
  const generatedBlock = t__namespace.variableDeclaration(
    "const",
    [t__namespace.variableDeclarator(id, t__namespace.callExpression(
      getImportIdentifier(ctx, path, HIDDEN_IMPORTS.compiledBlock[ctx.serverMode]),
      [
        newComponent,
        t__namespace.objectExpression(options)
      ]
    ))]
  );
